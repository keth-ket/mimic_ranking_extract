...
const processData = (textData: string[], nameMap: NameMap) => {
  for (let i = 0; i < textData.length; i++) {
    ...
    const cleanedValue =
      valueStr.split(" ").pop()?.replace(",", "").replace("M", "") ?? "";
...
const mostRepeatedNumber = (nameMap: NameMap): Record<string, number> => {
  ...
  for (const name in nameMap) {
    if (nameMap[name].length > 0) {
      ...
      const mostCommon = Object.entries(countMap).reduce((a, b) =>
        b[1] > a[1] ? b : a
      );
...
export default function TextExtract() {
      ...
      await Tesseract.recognize(image, "eng", {})
        .then(({ data: { text } }) => {
          const lines = text
            .split("\n")
            .map((line) => line.trim())
            .filter((line) => line !== ""); // Split and clean up
  ...
  return (
    <div className="flex w-full flex-col bg-accent h-full overflow-auto p-10 items-center gap-6">
      ...
      {images?.length > 0 && (
        <div className="flex flex-col items-center gap-6">
          <Button
            ...
            disabled={extractingText || Object.keys(members).length <= 0}
          >
          ...
          <div className="flex flex-row gap-4">
            {attacked != "" && (
              <pre className="border border-black rounded-lg p-2">
                {attacked}
              </pre>
            )}
            {score != "" && (
              <pre className="border border-black rounded-lg p-2">{score}</pre>
            )}
          </div>
        </div>
      )}
